...
autoencoder.load_state_dict(torch.load('/home/hkj/yolov10pj/yolov10_human/pytorch-Deep-SVDD/weights/pretrained_parameters.pth', map_location=device))

# Deep SVDD 하이퍼스피어 중심 로드
checkpoint = torch.load('/home/hkj/yolov10pj/yolov10_human/pytorch-Deep-SVDD/weights/final_deep_svdd.pth', map_location=device)
# 저장된 checkpoint 파일에서 키를 확인
#checkpoint = torch.load('/home/hkj/yolov10pj/yolov10_human/pytorch-Deep-SVDD/weights/pretrained_parameters.pth', map_location=device)
...
def predict_and_save(image_path, save_directory):
    ...
    # 이 부분은 저장된 이미지에서 얼굴을 감지한 후 이상 탐지를 수행하는 로직입니다
    for result_file in results.files:

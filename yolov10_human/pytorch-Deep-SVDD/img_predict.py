...
def predict_yolo(model, image_path):
    results = model.predict(image_path, save=True, imgsz=640, conf=0.5, device=1)
    return results
...
def draw_results(image_path, boxes, anomalies, output_path):
    ...
    #try:
    #    font = ImageFont.truetype("arial.ttf", font_size)  # 시스템에 Arial 폰트가 설치되어 있어야 함
    #except IOError:
        # 폰트 파일을 찾을 수 없을 때 기본 폰트 사용
...
def main():
    ...
    yolo_model_path = '/home/hkj/yolov10pj/yolov10_human/trainresult/result_facedetect3/weights/best.pt'
    ...
    for image_path in image_paths:
        ...
        # 이상 탐지 임계값 설정
        threshold = 0.05  # 기존의 0.5에서 더 낮춰 설정

...
def load_deep_svdd(model_path, autoencoder, device):
    ...
    center = torch.tensor(svdd_center_radius['center'], dtype=torch.float32).to(device)
    radius = torch.tensor(svdd_center_radius['radius'], dtype=torch.float32).to(device)
    return DeepSVDD(autoencoder, center, radius).to(device)

# YOLOv10 모델로 객체 감지
def detect_objects(image_path, device):
    yolo_model = YOLOv10().to(device)

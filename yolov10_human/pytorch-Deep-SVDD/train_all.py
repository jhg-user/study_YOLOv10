...
class TrainerDeepSVDD:
    ...
    def save_weights_for_DeepSVDD(self, model, dataloader):
        ...
        torch.save({'center': c.cpu().data.numpy().tolist(), 'net_dict': net.state_dict()}, 'pretrained_parameters3.pth')
    ...
    def train(self):
        ...
        if self.args.pretrain:
            state_dict = torch.load('pretrained_parameters3.pth')
        ...
        self.save_model_and_center('final_deep_svdd3.pth')
    ...
    def save_checkpoint(self, epoch, path='checkpoint3.pth'):
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(os.path.dirname(path))
        torch.save({
            'epoch': epoch,
...
if __name__ == "__main__":
    ...
    args = {
        ...
        'lr_ae': 1e-3,
        'batch_size': 8,
        'weight_decay_ae': 1e-5,
        'num_epochs_ae': 10,
        'lr': 1e-3,
        'weight_decay': 1e-5,
        'num_epochs': 50,
        'checkpoint_interval': 10,
        'tensorboard_log_dir': 'runs/DeepSVDD_experiment'
    }

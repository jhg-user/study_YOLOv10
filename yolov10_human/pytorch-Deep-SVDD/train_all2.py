...
class TrainerDeepSVDD:
    ...
    def pretrain(self):
        ...
        torch.save(ae.state_dict(), 'autoencoder10.pth')
    ...
    def save_weights_for_DeepSVDD(self, model, dataloader):
        ...
        torch.save({'center': c.cpu().data.numpy().tolist(), 'net_dict': net.state_dict()}, 'pretrained_parameters10.pth')
    ...
    def train(self):
        ...
        if self.args.pretrain:
            state_dict = torch.load('pretrained_parameters10.pth')
        ...
        self.save_model_and_center('./weights/final_deep_svdd10.pth')
    ...
    def save_checkpoint(self, epoch, path='./weights/checkpoint10.pth'):
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(os.path.dirname(path))
        torch.save({
            'epoch': epoch,
...
if __name__ == "__main__":
    ...
    #parser.add_argument('--normal_class', type=str, default='human-face', help='Class to be treated as normal. The rest will be considered as anomalous.')
    parser.add_argument('--checkpoint_interval', type=int, default=10, help='Interval for saving checkpoints')
    parser.add_argument('--tensorboard_log_dir', type=str, default='runs/DeepSVDD_experiment10', help='Directory for TensorBoard logs')

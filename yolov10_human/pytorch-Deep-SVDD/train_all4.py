...
class TrainerDeepSVDD:
    ...
    def pretrain(self):
        ...
        torch.save(ae.state_dict(), os.path.join(self.args.result_dir, 'autoencoder7.pth'))
    ...
    def save_weights_for_DeepSVDD(self, model, dataloader):
        ...
        torch.save({'center': c.cpu().data.numpy().tolist(), 'net_dict': net.state_dict()}, os.path.join(self.args.result_dir, 'pretrained_parameters7.pth'))
    ...
    def train(self):
        ...
        if self.args.pretrain:
            state_dict = torch.load(os.path.join(self.args.result_dir, 'pretrained_parameters7.pth'))
        ...
        self.save_model_and_center(os.path.join(self.args.result_dir, 'final_deep_svdd7.pth'))
    ...
    def save_checkpoint(self, epoch, path=None):
        if path is None:
            path = os.path.join(self.args.result_dir, f'checkpoint_epoch7_{epoch}.pth')
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(os.path.dirname(path))
        torch.save({
            'epoch': epoch,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr_milestones', type=int, nargs='+', default=[50, 100], help='스케줄러가 학습률을 0.1로 줄이는 마일스톤')
    ...
    parser.add_argument('--result_dir', type=str, default='./results', help='결과 파일 저장 디렉토리')
    ...
    # 사용할 GPU 선택 (가장 첫 번째 사용 가능한 GPU 선택)

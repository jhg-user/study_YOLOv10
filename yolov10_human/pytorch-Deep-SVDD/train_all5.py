...
class TrainerDeepSVDD:
    ...
    def pretrain(self):
        ...
        torch.save(ae.state_dict(), 'autoencoder11.pth')
        self.save_weights_for_DeepSVDD(ae, self.train_loader)
        print(f'autoencoder save')
    ...
    def save_weights_for_DeepSVDD(self, model, dataloader):
        ...
        torch.save({'center': c.cpu().data.numpy().tolist(), 'net_dict': net.state_dict()}, 'pretrained_parameters11.pth')
    ...
    def train(self):
        ...
        if self.args.pretrain:
            state_dict = torch.load('pretrained_parameters11.pth')
        ...
        self.save_model_and_center('./weights/final_deep_svdd11.pth')
    ...
    def save_checkpoint(self, epoch, path='./weights/checkpoint11.pth'):
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(os.path.dirname(path))
        torch.save({
            'epoch': epoch,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', type=float, default=1e-5, help='learning rate')  # 학습률 조정
    ...
    parser.add_argument('--lr_ae', type=float, default=1e-5, help='learning rate for autoencoder')
    ...
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size")  # 배치 크기 조정
    ...
    parser.add_argument('--tensorboard_log_dir', type=str, default='runs/DeepSVDD_experiment11', help='Directory for TensorBoard logs')
